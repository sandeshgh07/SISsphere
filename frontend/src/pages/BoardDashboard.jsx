        ...
        const loadData = async () => {
            try {
                const [analyticsRes, financeRes] = await Promise.all([
                    fetch(`${import.meta.env.VITE_BACKEND_URL}/api/board/analytics`, {
                        headers: { Authorization: `Bearer ${token}` }
                    }),
                    fetch(`${import.meta.env.VITE_BACKEND_URL}/api/board/financial-velocity`, {
                        headers: { Authorization: `Bearer ${token}` }
                    })
                ]);

                if (analyticsRes.ok) setAnalytics(await analyticsRes.json());
                if (financeRes.ok) setFinancials(await financeRes.json());
    ...
    return (
        <div className="space-y-8 p-4 md:p-8 bg-slate-50 min-h-screen">
            <div className="flex justify-between items-center">
                 <h2 className="text-3xl font-bold text-[#003333]">Executive Dashboard</h2>
            </div>
