...
const StatCard = ({ title, value, icon: Icon, change, changeType, subtitle, color }) => (
    <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100 min-w-0">
        <div className="flex items-start justify-between gap-2">
            ...
            {change && (
                <span className={`text-sm font-medium flex items-center gap-1 flex-shrink-0 ${changeType === 'up' ? 'text-green-500' : 'text-red-500'}`}>
                    ...
                    <tbody>
                        {schools.slice(0, 5).map((school) => (
                            <tr key={school.id} className="border-b last:border-0">
                                <td className="py-3 font-medium text-gray-800 max-w-[150px] truncate" title={school.name}>{school.name}</td>
                                <td className="py-3">
    ...
    if (error) {
        return (
            <div className="bg-red-50 border border-red-200 rounded-xl p-8 text-center m-6">
                ...
                <p className="text-sm text-red-500 mt-2">Failed to load dashboard data.</p>
            </div>
        );
    }
...
export default GodViewOverview;
